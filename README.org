...
*** TODO Remove dependencies (e.g. no need of nodemon anymore)
*** TODO Make it strippable Typescript files
*** TODO Use a compose file instead of huge build script
*** TODO Adding Node permissions features
** Podman container

To build and run the container with Podman:
#+begin_src shell
  podman build -t fastify-sqlite-server .
  podman run -p 8080:8080 --user 1001 fastify-sqlite-server
#+end_src

This Containerfile:
- Uses Node.js 16 Alpine as a lightweight base image
- Creates and sets up a non-root `node` user for security
- Properly handles permissions and ownership
- Copies package files first for efficient caching during rebuilds
- Installs dependencies before copying the full application code
- Exposes port 8080
- Runs the server with node directly (not using nodemon as we're in production)
** Development mode
This is not meant to be pushed in production at all.
So the work is to make it reproducible (pinned versions).

### **Step 2: Create a `.podmanignore` file**

This file will exclude unnecessary files during the build process:
...
Run this command to build your container image:
...
### **Step 4: Run the container for development**

For development, you want:
- Live reloading with `nodemon`
- Volume mounts to sync code changes between your host and container
- Access to debugging tools if needed
...
Assuming your `package.json` has a `"dev"` script like this:
...
### **Step 5: (Optional) Use a Compose File for Orchestration**

If you prefer using `podman-compose`, create a `podman-compose.yml` file:
...
- **Volume Mounting**: Changes to your local code are immediately reflected in the container.
- **Live Reloading**: `nodemon` restarts your server when files change.
- **Consistent Environment**: Same base image and dependencies for both development and production.
- **Security**: Running as a non-root user even in development.

---

### **Step 6: Rebuild the Image When Needed**

If you make changes to your `package.json` or other dependencies, rebuild the image:
...
This setup allows you to work seamlessly between development and production while maintaining consistency in your environment.
