...
* Working with JSON
Working with the package [[https://sqlite.org/json1.html][json1]].
Load the package in *sqlite3* REPL and call the ~json()~ function:

#+begin_src sql
sqlite> .load /Users/xavierbrinon/Documents/SQLite/Complete-Intro-to-SQLite/sqlite-json1/json1
sqlite> select json('{"username": "xbrinon", "favourites": ["Daft Punk", "Radiohead"]}');
{"username":"xbrinon","favourites":["Daft Punk","Radiohead"]}
...
** Accessors
If I want to access a value from a specific fields in the JSON from sqlite: 'json -> json'
#+begin_src sql
sqlite> select json('{"username": "xbrinon", "favourites": ["Daft Punk", "Radiohead"]}') -> 'username';
"xbrinon"
#+end_src

~"xbrinon"~ here is a JSON object of a string.
To get the value, ~xbrinon~, there is '->>'
#+begin_src sql
sqlite> select json('{"username": "xbrinon", "favourites": ["Daft Punk", "Radiohead"]}') ->> 'username';
xbrinon
#+end_src
...
** Binary JSON
To store efficiently JSONs, use the binary format; more efficient but not human
readable.
#+begin_src sql
sqlite> select json('{"username": "xbrinon", "favourites": ["Daft Punk", "Radiohead"]}');
{"username":"xbrinon","favourites":["Daft Punk","Radiohead"]}
sqlite> select jsonb('{"username": "xbrinon", "favourites": ["Daft Punk", "Radiohead"]}');
?2?usernamewxbrinon?favourites??Daft Punk?Radiohead
#+end_src

JSONB is more efficient in a closed loop system but inserting and using JSON
from outside *sqlite* use ~json()~

** Operations
You let *sqlite* do the queries, destructuring, and construct.
#+begin_src sql
sqlite> CREATE TABLE users (email, data);
...
Album          Employee       InvoiceLine    PlaylistTrack
Artist         Genre          MediaType      Track        
Customer       Invoice        Playlist       users        
sqlite> .schema users
CREATE TABLE users (email, data);
sqlite> select data from users;
?=?favorites??Daft Punk?RadioheadGname?WfirstWBrianGlastGHolt
?1?favorites??Daft PunkGname?Wfirst7BobGlastWSmith
?EWadmin?favorites??The BeatlesWQueenGname?WfirstWAliceGlastwJohnson
?>?favorites?wNirvana?Pearl JamGname?WfirstwCharlieGlastWBrown
?C?favorites??Pink Floyd?
                         Led ZeppelinGname?WfirstGDaveGlastgWilson
?A?favorites?wMadonna?Michael JacksonGname?Wfirst7EveGlastWDavis
?=?favorites?WQueen?David BowieGname?WfirstWFrankGlastgMiller
?@?favorites??Radiohead?
                        Led ZeppelinGname?WfirstWGraceGlast7Lee
'U2?RadioheadGname?WfirstGHankGlastgTaylor
?:?favorites?WAdele?BeyoncéGname?Wfirst7IvyGlast?Anderson
?9?favorites??RadioheadGMuseGname?WfirstGJackGlastgThomas
?B?favorites??
              Taylor SwiftwMadonnaGname?WfirstGKateGlast?Martinez
?;?favorites?wNirvana?Daft PunkGname?Wfirst7LeoGlastgGarcia
sqlite> SELECT data -> 'name' ->> 'first', data -> 'name' ->> 'last' FROM users;
Brian|Holt
Bob|Smith
Alice|Johnson
Charlie|Brown
...
Kate|Martinez
Leo|Garcia
sqlite> SELECT data -> 'name' ->> 'first', data -> 'name' ->> 'last' FROM users WHERE json_array_length(data, '$.favorites') < 2;
Bob|Smith
#+end_src


** Adding to a jsonb table
What are Brian's favs?
#+begin_src sql
sqlite> select data ->> 'favorites' from users where email = 'brian@example.com';
["Daft Punk","Radiohead"]
#+end_src

Now, let's add 'The XX' as new favorite.
#+begin_src sql
sqlite> update users
   ...> set data = jsonb_insert(
(x1...>   (select data from users where email = 'brian@example.com'),
(x1...>   '$.favorites[#]',
(x1...>   'The XX'
(x1...>   )
   ...> where email = 'brian@example.com';
sqlite> select data ->> 'favorites' from users where email = 'brian@example.com';
["Daft Punk","Radiohead","The XX"]
#+end_src

** Complex query
What is the most favourited band?
#+begin_src sql
select count(Favorites.value) as count, Favorites.value
from users, json_each(data ->> 'favorites') as Favorites
group by Favorites.value
order by count desc;
4|Radiohead
3|Daft Punk
2|Queen
